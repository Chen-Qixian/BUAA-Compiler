    ...
    if(sy == IDEN){
        ...
        if(sy == LZKH){ // 数组
            ...
            if(idx_type){
                ...
                if(str_is_num(tmp)){
                     if(tab[idx_in_tab].len > 0 && (str2int(tmp) >= tab[idx_in_tab].len || str2int(tmp) < 0)){
                        error(46); // 数组越界
                     }
                }
            }
            ...
            if(idx_in_tab == -1){
                ...
                if(tab[idx_in_tab].type == chariden) is_char = true;
                if(tab[idx_in_tab].obj == constobj && tab[idx_in_tab].type == intiden) return int2str(tab[idx_in_tab].adr);
            }
            result = Name;
        }
    }
...
//     // 定义常量传播使用的结构体数组：体现* /交换律，对于变量的存储
//    struct variables{
//        opration opr;
//        std::string vname;
//    }vars[VARMAX];
//    int var_idx = 0;  // index for vars
    x = factor(is_char); // 入口为item_beg_set中的元素，item和factor的beg_set相同；出口为factor的下一个符号
//    if(!str_is_num(x)){
//        vars[var_idx].opr = mult;
//        vars[var_idx].vname = x;
//        var_idx ++;
//        x = "1";
//    }
...
//        if(!str_is_num(y)){
//            vars[var_idx].opr = op;
//            vars[var_idx].vname = y;
//            var_idx ++;
//        }
        if(str_is_num(x) && str_is_num(y)){
            int xnum,ynum,res;
            xnum = str2int(x);
            ynum = str2int(y);
            if(op == mult){
...
std::string expression(bool& is_char){
    ...
    // 定义常量传播使用的结构体数组：体现加法交换律，对于变量的存储
    struct variables{
        opration opr;
        std::string vname;
    }vars[VARMAX];
...
//        }
//    }
    if(flag){ // 如果有+-那么一定不是char型
        is_char = false;
        x = item(place_holder);
        if(str_is_num(x) && op == sub){
            if(x[0] == '-'){
    ...
    if(sy == LZKH){
        ...
        if(idx_type){
            ...
            if(str_is_num(arr_idx)){
                if(tab[idx_in_tab].len > 0 &&(str2int(arr_idx) >= tab[idx_in_tab].len || str2int(arr_idx) < 0) ){
                    error(46); // 数组越界
                }
            }
        }
...
void statement(){
    ...
    //std::cout << "in find sym: " << id << std::endl;
...
// 语法分析阶段输出
void grammar_output(){
    grammar_analysis_result.open("16061160_check_grammar.txt");
    mid_code_result.open("16061160_优化后中间代码.txt");
