...
void loadOp(std::string op , std::string reg){       // 根据op加载值到寄存器 【数组未检查】
    ...
    if(MOT == imm_midop){
    ...
    else if(MOT == global_arr_midop){           // 全局数组
        ...
        if(mot == imm_midop){        // 数组下标是常量
            ...
            enter_mips_5(mlw , "$t3" , "$fp" , off);          // off 为下标索引在函数中的偏移量 lw $t0 off($fp)
            enter_mips_4(mmul , "$t3" , "$t3" , 4);           // mul $t0 $t0 4
            enter_mips_4(maddi , "$t3" , "$t3" , offset);     // offset 为数组的首地址  addi $t0 $t0 offset
            enter_mips_5(mlw , reg , "$t3" , 0);              // lw reg 0($t0)
        }
        else if(mot == global_var_midop){  // 数组下标是全局变量
            enter_mips_2(mlw , "$t3" , off);           // 不同于上文，直接从地址中取数
            enter_mips_4(mmul , "$t3" , "$t3" , 4);           // mul $t0 $t0 4
            enter_mips_4(maddi , "$t3" , "$t3" , offset);     // offset 为数组的首地址  addi $t0 $t0 offset
            enter_mips_5(mlw , reg , "$t3" , 0);              // lw reg 0($t0)
        }
        ...
        if(mot == imm_midop){        // 数组下标是常量
            ...
            enter_mips_4(mmul , "$t3" , "$t3" , -4);          // mul $t0 $t0 -4
            enter_mips_4(maddi , "$t3" , "$t3" , offset);     // offset 为数组的首地址  addi $t0 $t0 offset
            enter_mips_7(madd , "$t3" , "$t3" , "$fp");       // add $t0 $t0 $fp  找到该元素位置
            enter_mips_5(mlw , reg , "$t3" , 0);              // lw reg 0($t0)
        }
        else if(mot == global_var_midop){  // 数组下标是全局变量
            enter_mips_2(mlw , "$t3" , off);           // 不同于上文，直接从地址中取数
...
void pushOp(){  // 【未完成】
    ...
    else if(midop_analyse(op , offset) == arr_midop){           // 局部数组
        std::string idx = op.substr(op.find("[") + 1 , op.find("]") - op.find("[") - 1);   // 获取数组下标索引
        int off;
        midoptyp mot = midop_analyse(idx , off);
        if(mot == imm_midop){        // 数组下标是常量
    ...
    else if(midop_analyse(op , offset) == global_arr_midop){     // 全局数组
        std::string idx = op.substr(op.find("[") + 1 , op.find("]") - op.find("[") - 1);   // 获取数组下标索引
        int off;
        midoptyp mot = midop_analyse(idx , off);
        if(mot == imm_midop){        // 数组下标是常量
...
void mid2mips(){
    mips_code_result.open("16061160_优化前目标代码.txt");
    gen_mips_str();
